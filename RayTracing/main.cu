...
void renderScene(Sphere* spheres, int numSpheres, Plane* planes, int numPlanes, Light* lights, int numLights, unsigned char* image, int width, int height) {
    ...
    // Выделение статической памяти (управляемая не работает на моей версии CUDA почему-то)
    checkCuda(cudaMalloc(&d_spheres, sizeof(Sphere) * numSpheres));
    checkCuda(cudaMalloc(&d_planes, sizeof(Plane) * numPlanes));
    checkCuda(cudaMalloc(&d_lights, sizeof(Light) * numLights));
    checkCuda(cudaMalloc(&d_image, sizeof(unsigned char) * width * height * 3));
...
int main() {
    ...
    Sphere spheres[] = {
    {{-2.0f, -0.0f, -6.0f}, 1.0f, {1.0f, 1.0f, 0.0f}}, 
    {{0.0f, 1.0f, -8.0f}, 2.0f, {0.0f, 1.0f, 1.0f}},  
    {{1.5f, -0.0f, -6.0f}, 1.0f, {0.0f, 1.0f, 0.0f}}  
    };
